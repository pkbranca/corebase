var TermTable = function () {
    ...
    var options = {
        data: {
            source: {
                read: {
                    method: "GET",
                    url: "/admin/dedicationteacher/get".proto().parseURL()
                }
            }
        },
        columns: [
            ...
            {
                field: "maxLessonHours",
                title: "Max Horas Lectivas"
            },
            {
                field: "minLessonHours",
                title: "Min Horas Lectivas"
            },
            {
                field: "maxNoLessonHours",
                title: "Max Horas No Lectivas"
            },
            {
                field: "minNoLessonHours",
                title: "Min Horas No Lectivas"
            },
            {
                field: "status",
                title: "Estado",
    ...
    var loadTermData = function (id) {
        $.ajax({
            url: ("/admin/dedicationteacher/get/" + id).proto().parseURL(),
            type: "GET",
            dataType: "json",
            success: function (result) {
                $("#eId").val(result.id);
                $("#eName").val(result.name);
                $("#eMinNoLessonsHours").val(result.minNoLessonHours);
                $("#eMinLessonsHours").val(result.minLessonHours);
                $("#eMaxNoLessonsHours").val(result.maxNoLessonHours);
                $("#eMaxLessonsHours").val(result.maxLessonHours);
                if (result.status)
    ...
    var deleteConfirmation = function (id) {
        swal({
            title: "¿Está seguro?",
            text: "La dedicación de docente será eliminado permanentemente",
            type: "warning",
            ...
            if (result.value) {
                $.ajax({
                    url: "/admin/dedicationteacher/eliminar",
                    type: "POST",
                    data: {
                        id: id
                    },
                    ...
                    error: function () {
                        toastr.error("La dedicación de docente tiene información relacionada", _app.constants.toastr.title.error);
                    }
                });
            }
        });
