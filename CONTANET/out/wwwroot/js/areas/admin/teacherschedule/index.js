    ...
    var init = function () {
        $.ajax({
            ...
            $("#btnQuery").on("click", function (e) {
                calendar.load($(".select2-teachers").val());
            });
            //$(".select2-teachers").on("change", function (e) {
            //    calendar.load($(".select2-teachers").val());
            //});
            if ($(".select2-teachers").val())
                calendar.load($(".select2-teachers").val());
        });
    }
    ...
    var form = {
        detail: {
            load: function (id) {
                mApp.blockPage();
                $.ajax({
                        url: `/admin/horarios/${$(".select2-teachers").val()}/${id}/get`.proto().parseURL()
                    })
                    .always(function () {
                        mApp.unblockPage();
                    })
