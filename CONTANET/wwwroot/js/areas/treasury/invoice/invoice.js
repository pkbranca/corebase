        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row, index) {
                    ...
                    return '<input hidden name="detail[' + index + '].Id" value="' + row.id + '"><div class="input-group btn--icon-only" style="max-width: 200px"><input type="text" name="detail[' + index + '].Cost" class="form-control m-detail-value m--align-right input-cost" placeholder= "0.00" value="' + row.cost.toFixed(2) + '"><div class="input-group-append"><button class="btn btn-primary m-btn m-btn--icon m-btn--icon-only" type="button" onclick="InvoiceDetailTable.updateCost(this)" data-id="' + row.id + '"><i class="la la-save"></i></button></div></div >';
                }
            }
        ]
    }

    var updateCost = function (btn) {
        var id = $(btn).data("id");
        ...
        $.ajax({
                url: "/pagos/recibo/detalle/post".proto().parseURL(),
                type: "POST",
                data: {
                    id: id,
                    newAmount: amount
            ...
            .done(function (result) {
                $("#total_amount").text(result.total.toFixed(2));
                input.val(result.cost.toFixed(2));
            })
            .fail(function (e) {
                input.val(e.responseJSON.cost.toFixed(2));
                toastr.error(e.responseJSON.error, _app.constants.toastr.title.error);
            })
            .always(function () {
